...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(5):
            image_loader.init_data_loader(sess)
            while True:
                try:
                    ...
                    cv2.imwrite("train_{}.jpg".format(i+1), img)
                    break
            image_loader.init_data_loader(sess, False)
            while True:
                try:
                    img, anno = sess.run([val_img, val_anno])
                except tf.errors.OutOfRangeError:
                    img = img[0]
                    img = img * 128 + 128
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    cv2.imwrite("val_{}.jpg".format(i + 1), img)
                    break
